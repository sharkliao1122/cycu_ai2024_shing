...
df_雙向 = pd.read_excel("C:\專題EXCEL新版\專題EXCEL\TCU052設計\TCU052設計水平雙向樑中點位移.xlsx", header=1)
df_三向 = pd.read_excel("C:\專題EXCEL新版\專題EXCEL\TCU052設計\TCU052設計 三向主樑中點位移.xlsx", header=1)
...
# 獲取輸入文件的資料夾路徑
input_folder = os.path.dirname("C:\專題EXCEL新版\專題EXCEL\TCU052設計\TCU052設計水平雙向樑中點位移.xlsx")

# 繪製雙向折線圖 3張折線圖 (U1, U2, U3)
for col in ["U1", "U2", "U3"]:
    plt.figure()
    plt.plot(df_雙向["StepNum"], df_雙向[col], label="Biaxial")
...
for col in ["U1", "U2", "U3"]:
    plt.figure()
    plt.plot(df_三向["StepNum"], df_三向[col], label="Triaxial")
